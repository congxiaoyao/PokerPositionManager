...
import com.congxiaoyao.util.asIterable
...
class HandCardModelTest {
    ...
    private fun move(selected: List<String>, dis: Int) {
        ...
        val locationManager = HandCardModel<String>(30, 100, 10)
        ...
        while (true) {
            move(locationManager, entities, beans, testManager, -dis)
            if (testManager.selected!!.last().location < -width) {
                break
            }
        }
    }

    private fun move(locationManager: HandCardModel<String>,
                     entities: List<HandCardModel.Entity<String>>,
                     beans: List<TestLocationManager.Bean>,
                     testManager: TestLocationManager,
                     dis: Int) {
        locationCast += measureTime {
            locationManager.aggregate(entities.first())
            locationManager.moveAndSort(dis)
        }
    ...
    private fun eq(locationManager: HandCardModel<String>, testManager: TestLocationManager) {
        val result1 = locationManager.entities.asIterable().map { TestLocationManager.Bean(it.tag, it.location) }
